...
public class WorkFlowImportRequestBodyDeserializer extends UllJsonDeserializer<WorkFlowImportRequestBody> {
    ...
    @Override
    public WorkFlowImportRequestBody parse(JsonNode flowNode) {
        validate(flowNode);
        UUID flowId = UUID.fromString(flowNode.get(JsonFields.ID).asText());
        ...
        for (JsonNode nodeJson : jsonNodes) {
            ...
            try {
                ...
                throw new RuntimeException("Failed to parse node content for node: " + nodeName, e);
            }
            Node node = new Node(
                    nodeName,
        ...
        WorkFlowImportRequestBody parsedRequest = new WorkFlowImportRequestBody();
        parsedRequest.setId(flowId);
